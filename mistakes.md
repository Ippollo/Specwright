...
**What happened**: Logged a job opportunity to Firestore (`users/{uid}/jobs`) when job tracking has moved to `C:\Workspace\Notepad\60_Career\jobs\` as markdown files.
**Why it was wrong**: Stale assumption — Job Hunter Firestore is no longer the tracking system for active job opportunities.
**Do instead**: Create a markdown file in `C:\Workspace\Notepad\60_Career\jobs\{Company} - {Title}.md` using the established frontmatter schema (date, company, title, status, source, match_score, etc.). Match the format of existing files in that folder.
